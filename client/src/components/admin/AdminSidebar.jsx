...
const AdminSidebar = () => {
    ...
    if (item.dropdown) {
      return (
        <div key={item.title}>
          <button
            onClick={(e) => toggleDropdown(item.dropdown, e)}
            className={`w-full flex items-center justify-between ${paddingLeft} py-3 transition-colors duration-200 text-gray-800 hover:text-white hover:bg-[#d9a82e]`}
          >
            <div className="flex items-center space-x-3">
              {item.icon && <item.icon size={20} />}
              <span className={textSize}>{item.title}</span>
      ...
      return (
        <Link
          ...
          className={`block ${paddingLeft} py-2 ${textSize} transition-colors duration-200 ${
            isActive(item.path)
              ? "bg-[#2377c1] text-white border-r-4 border-[#d9a82e]"
              : "text-gray-800 hover:text-white hover:bg-[#d9a82e]"
          }`}
        >
          {item.title}
        </Link>
  ...
  return (
    <div className="w-64 bg-white shadow-lg h-screen overflow-y-auto fixed left-0 top-0 z-50 border-r border-[#eaf3fb]">
      <div className="p-6 border-b ">
        <Link to="/admin/dashboard" className="flex items-center ">
          <img src="/baitulProtein/Baitullogo.webp" alt="Admin" className="" />
        </Link>
      </div>
