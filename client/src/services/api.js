...
export const adminAPI = {
  ...
  // Admin Orders - full list and mutations
  getOrders: (params = {}) => {
    const qs = new URLSearchParams(params).toString()
    return apiRequest(`/api/admin/orders${qs ? `?${qs}` : ""}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
    })
  },
  ...
  updateOrderTracking: (id, trackingId) =>
    apiRequest(`/api/admin/orders/${id}/tracking`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
      body: JSON.stringify({ trackingId }),
    }),
  notifyOrder: (id) =>
    apiRequest(`/api/admin/orders/${id}/notify`, {
      method: "POST",
      headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
    }),
}
