...
const Cart = () => {
      ...
      <li key={`${item._id}-${bundleId || 'standalone'}`} className="p-3">
        ...
        <div className="block sm:hidden">
          <div className="flex flex-row items-center mb-3">
            ...
            <div className="flex-1 ml-4">
              ...
              <p className="mt-1 text-sm text-gray-500">{item.brand?.name || 'N/A'}</p>
                {isInBundle && (
                  <p className="mt-1 text-xs font-medium text-[#d9a82e]">
          ...
          <div className="flex flex-row items-center justify-between">
            <div
              className="flex items-center rounded-md w-max overflow-hidden border"
              style={{ backgroundColor: '#2377c1', borderColor: '#1d66a5' }}
            >
              <button
                onClick={() => handleQuantityChange(item._id, item.quantity - 1, bundleId)}
                className="px-4 py-2 text-white bg-[#2377c1] hover:brightness-95 disabled:opacity-60"
                disabled={item.quantity === 1}
              >
                <Minus size={16} />
              </button>
              <span className="px-5 py-2 text-white border-x" style={{ borderColor: '#1d66a5' }}>{item.quantity}</span>
              <button
                onClick={() => handleQuantityChange(item._id, item.quantity + 1, bundleId)}
                className="px-4 py-2 text-white bg-[#2377c1] hover:brightness-95"
              >
                <Plus size={16} />
              </button>
            </div>
          ...
          <div className="sm:ml-8 flex-1">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                ...
                {isInBundle && (
                  <p className="text-xs font-medium mb-2 text-[#d9a82e]">
                    {item.bundleDiscount ? "Bundle Item (25% OFF)" : "Bundle Item"}
                  </p>
                )}
            ...
            <div className="flex items-center justify-between">
              <div
                className="flex items-center rounded-md overflow-hidden border shadow"
                style={{ backgroundColor: '#2377c1', borderColor: '#1d66a5', }}
              >
                <button
                  onClick={() => handleQuantityChange(item._id, item.quantity - 1, bundleId)}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {/* Header scrolls with page on all breakpoints */}
      <div className="mb-4 md:mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          {/* Keep title on a single line on mobile */}
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 whitespace-nowrap">Shopping Cart</h1>
          {/* Buttons inline on mobile, below the title */}
          <div className="mt-2 md:mt-0 flex flex-nowrap space-x-2 md:space-x-3">
            <button
              onClick={handleOpenCouponsModal}
              className="px-2 py-2 lg:px-5 lg:py-2 text-sm font-medium rounded-md transition-colors flex items-center shadow whitespace-nowrap"
              style={{ backgroundColor: '#d9a82e', color: 'white', }}
            >
            ...
            <Link
              to="/"
              className="px-2 py-2 lg:px-5 lg:py-2 text-sm font-medium text-white rounded-md shadow whitespace-nowrap"
              style={{ backgroundColor: '#2377c1', }}
            >
              Continue Shopping
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {Object.values(grouped).map((bundle) => {
              ...
              return (
                <div key={bundle.bundleId} className="bg-white rounded-lg shadow-sm border-2 mb-6 border-[#d9a82e]">
                  {/* Bundle Header */}
                  <div className="px-6 py-3 border-b border-[#d9a82e] bg-[rgba(217,168,46,0.08)]">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
                        <Package className="mr-2 text-[#d9a82e]" size={20} />
                        <h3 className="text-lg font-semibold text-[#d9a82e]">Frequently Bought Together</h3>
                      </div>
                      <button
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-md p-6 shadow-[0_10px_25px_-10px_rgba(217,168,46,0.45)]">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {!coupon ? (
                    <button
                      className="text-white px-3 py-1 rounded disabled:opacity-50 shadow"
                ...
                <div className="border-t pt-4 flex justify-between font-medium">
                  ...
                  <span className="text-black hover:text-[#d9a82e]">{formatPrice(totalWithDeliveryTaxCoupon)}</span>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="mb-4">
                  <label className="flex items-start space-x-2 text-sm text-gray-600">
                    <input 
                      type="checkbox" 
                      className="mt-1 rounded border-gray-300 text-[#d9a82e] focus:ring-[#d9a82e]" 
                      defaultChecked 
                    />
                    <span>
                      I agree to our{' '}
                      <Link to="/terms" className="text-[#d9a82e] hover:brightness-95 underline">
                        Terms of use
                      </Link>
                      {' '}&{' '}
                      <Link to="/privacy" className="text-[#d9a82e] hover:brightness-95 underline">
                        Privacy Policy
                      </Link>
                    </span>
                  </label>
                ...
                <Link
                  to="/checkout"
                  className="w-full text-white font-medium py-3 px-4 rounded-md flex items-center justify-center transition-colors shadow"
