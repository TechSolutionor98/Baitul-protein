  ...
  return (
    <div className="relative inline-block text-left" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((s) => !s)}
        className="px-4 py-2 border border-[#d4af37] rounded-lg bg-[#d4af37] text-black focus:outline-none focus:ring-2 focus:ring-[#d4af37] flex items-center gap-2"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <span className="truncate max-w-[46vw] sm:max-w-none">{current}</span>
  ...
  return (
    <div className="">
      <Slider
        ...
        value={range}
        onChange={handleSliderChange}
        trackStyle={[{ backgroundColor: "#d4af37" }]} // gold
        handleStyle={[
      ...
      <div className="flex gap-2 mb-4">
        <input
          type="number"
          className="w-1/2 border rounded px-2 py-1 text-center focus:border-[#d4af37] focus:ring-[#d4af37]"
          value={inputMin}
          min={min}
          max={inputMax}
          onChange={handleInputMin}
      ...
      <button
        type="button"
  className="w-full bg-white border border-[#d4af37] text-[#d4af37] rounded py-2 font-semibold hover:bg-[#f7f0d8] hover:text-[#b89621] hover:border-[#b89621] transition"
        onClick={handleApply}
      >
        Apply
      </button>
...
const Shop = () => {
  ...
  // Derive selected category/subcategory objects for SEO and redirects
  const categoryObj = categories.find((cat) => cat._id === selectedCategory)
  const subcategoryObj =
  ...
  // Handle redirect if URL is set (must be before any early returns to keep hook order stable)
  useEffect(() => {
  ...
  }, [redirectUrl, navigate, location.pathname])
  ...
  // Get selected subcategory object (alias for clarity in render)
  const selectedSubCategoryObj = subcategoryObj
  ...
  // Badge under title: show Category or Brand based on how user arrived
  const urlCtx = parseShopURL(location.pathname, location.search)
  const getBrandFromParam = (param) =>
    brands.find((b) => b.name === param || createSlug(b.name) === createSlug(param))
  const brandFromUrl = urlCtx?.brand ? getBrandFromParam(urlCtx.brand) : null
  let badgeKind = null
  let badgeLabel = null
  if (brandFromUrl) {
    badgeKind = 'Brand'
    badgeLabel = brandFromUrl.name || urlCtx.brand
  } else if (selectedSubCategoryObj) {
    badgeKind = 'Category'
    badgeLabel = selectedSubCategoryObj.name
  } else if (categoryObj && categoryObj.name && selectedCategory !== 'all') {
    badgeKind = 'Category'
    badgeLabel = categoryObj.name
  } else if (urlCtx?.parentCategory && urlCtx.parentCategory !== 'all') {
    badgeKind = 'Category'
    badgeLabel = urlCtx.parentCategory.replace(/-/g, ' ')
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-4 md:hidden">
          <button
            className="flex md:hidden items-center gap-2 px-4 py-2 bg-[#2377c1] text-white rounded font-semibold shadow hover:bg-lime-600 transition"
            onClick={() => setIsMobileFilterOpen(true)}
          >
            <FilterIcon size={20} />
            Filter
            ...
            <div className="relative bg-white w-80 max-w-full h-full shadow-xl ml-auto animate-slideInRight flex flex-col z-20">
                    ...
                    <div className="mt-4 space-y-2">
                      <div className="flex items-center cursor-pointer" onClick={() => handleCategoryChange("all")}>
                        <div className="relative flex items-center">
                          ...
                          <div
                              className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 ${
                              selectedCategory === "all" ? "border-[#d4af37] bg-[#d4af37]" : "border-gray-300"
                            }`}
                          >
                            {selectedCategory === "all" && <div className="w-2 h-2 rounded-full bg-white"></div>}
                          </div>
                        </div>
                        <span className={`text-sm ${selectedCategory === "all" ? "text-[#d4af37] font-semibold" : "text-gray-700"}`}>All Categories</span>
                      </div>
                      ...
                      {categories.map((category) => (
                        ...
                        >
                          <div className="relative flex items-center">
                            ...
                            <div
                              className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 ${
                                selectedCategory === category._id ? "border-[#d4af37] bg-[#d4af37]" : "border-gray-300"
                              }`}
                            >
                              {selectedCategory === category._id && (
                                <div className="w-2 h-2 rounded-full bg-white"></div>
                          ...
                          <span className={`text-sm ${selectedCategory === category._id ? "text-[#d4af37] font-semibold" : "text-gray-700"}`}>{category.name}</span>
                        </div>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="mt-4 space-y-2">
                      {subCategories.map((subcat) => (
                        ...
                        >
                          ...
                          <span className={`text-sm ${selectedSubCategories[0] === subcat._id ? "text-[#d4af37] font-semibold" : "text-gray-700"}`}>{subcat.name}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="border-b pb-4">
                    ...
                    {showBrandFilter && (
                      <div className="mt-4 space-y-3">
                        ...
                        <div className="max-h-48 overflow-y-auto space-y-2 scrollbar-gold">
                          {filteredBrands.map((brand) => (
                            <div
                              key={brand._id}
                              className="flex items-center cursor-pointer"
                            ...
                            >
                              ...
                              <span className={`text-sm ${selectedBrands.includes(brand._id) ? "text-[#d4af37] font-semibold" : "text-gray-700"}`}>{brand.name}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    >
                      <div className="relative flex items-center">
                        ...
                        <div
                          className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 ${
                            stockFilters.inStock ? "border-[#d4af37] bg-[#d4af37]" : "border-gray-300"
                          }`}
                        >
                          {stockFilters.inStock && <div className="w-2 h-2 rounded-full bg-white"></div>}
                        </div>
                    ...
                    >
                      <div className="relative flex items-center">
                        ...
                        <div
                          className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 ${
                            stockFilters.outOfStock ? "border-[#d4af37] bg-[#d4af37]" : "border-gray-300"
                          }`}
                        >
                          {stockFilters.outOfStock && <div className="w-2 h-2 rounded-full bg-white"></div>}
                        </div>
                    ...
                    <div className="flex items-center cursor-pointer" onClick={() => handleStockFilterChange("onSale")}>
                      <div className="relative flex items-center">
                        ...
                        <div
                          className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 ${
                            stockFilters.onSale ? "border-[#d4af37] bg-[#d4af37]" : "border-gray-300"
                          }`}
                        >
                          {stockFilters.onSale && <div className="w-2 h-2 rounded-full bg-white"></div>}
                        </div>
              ...
              <div className="p-4 border-t bg-white flex items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setIsMobileFilterOpen(false)}
                      className="flex-1 px-4 py-2 bg-[#d4af37] text-black rounded-md font-semibold hover:brightness-95"
                >
                  Apply
                </button>
              </div>
            ...
            <div className="flex flex-row justify-between items-center mb-6 relative z-10">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex items-center gap-3 flex-wrap">
                  {badgeLabel && (
                    <span
                      className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium text-black border"
                      style={{ backgroundColor: '#f7f0d8', borderColor: 'rgba(217,168,46,0.4)' }}
                    >
